    ...
    func Newset() -> some View{
        VStack(spacing : 0){
            HStack(spacing : 0){
                ...
                Spacer()
               NavigationLink(destination: {
                   ConditionDynamicView(addPram: "&order_by=id+desc")
                       .environmentObject(interAd)
                       .environmentObject(rewardAd)
                       .environmentObject(store)
                }, label: {
            ...
            ZStack{
                if !wallpaperCatelogVM.dynamicNew.isEmpty{
                    ScrollView(.horizontal, showsIndicators: false){
                        HStack(spacing : 8){
                            Spacer().frame(width: 8)
                            ForEach(wallpaperCatelogVM.dynamicNew, id: \.id){
                                wallpaper in
                                let string : String = wallpaper.thumbnail?.path.preview ?? wallpaper.path.first?.path.small ?? ""
                                NavigationLink(destination: {
                                    SPWLOnePageDetailView(wallpaper: wallpaper)
    ...
    func Popular() -> some View{
        VStack(spacing : 0){
            HStack(spacing : 0){
                ...
                Spacer()
               NavigationLink(destination: {
                   ConditionDynamicView(addPram: "&order_by=daily_rating+desc,id+desc")
                       .environmentObject(interAd)
                       .environmentObject(rewardAd)
                       .environmentObject(store)
                }, label: {
            ...
            ZStack{
                if !wallpaperCatelogVM.dynamicPopular.isEmpty{
                    ScrollView(.horizontal, showsIndicators: false){
                        HStack(spacing : 8){
                            Spacer().frame(width: 8)
                            ForEach(wallpaperCatelogVM.dynamicPopular, id: \.id){
                                wallpaper in
                                let string : String = wallpaper.thumbnail?.path.preview ?? wallpaper.path.first?.path.small ?? ""
                                NavigationLink(destination: {
                                    SPWLOnePageDetailView(wallpaper: wallpaper)
